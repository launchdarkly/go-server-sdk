...
type UnboundedSegmentStoreTestSuite struct {
	...
	setKeysFn      func(prefix, userHashKey string, included []string, excluded []string) error
}
...
//
// The setMetadataFn and setKeysFn parameters are functions for populating the database.
func NewUnboundedSegmentStoreTestSuite(
	...
	setKeysFn func(prefix string, userHashKey string, included []string, excluded []string) error,
...
func (s *UnboundedSegmentStoreTestSuite) runUserMembershipTests(t testbox.TestingT) {
	...
	t.Run("includes only", func(t testbox.TestingT) {
		s.withStoreAndEmptyData(t, func(store interfaces.UnboundedSegmentStore) {
			require.NoError(t, s.setKeysFn("", fakeUserHash, []string{"key1", "key2"}, nil))
	...
	t.Run("excludes only", func(t testbox.TestingT) {
		s.withStoreAndEmptyData(t, func(store interfaces.UnboundedSegmentStore) {
			require.NoError(t, s.setKeysFn("", fakeUserHash, nil, []string{"key1", "key2"}))
	...
	t.Run("includes and excludes", func(t testbox.TestingT) {
		s.withStoreAndEmptyData(t, func(store interfaces.UnboundedSegmentStore) {
			require.NoError(t, s.setKeysFn("", fakeUserHash, []string{"key1", "key2"}, []string{"key2", "key3"}))
...
func assertEqualMembership(
	...
	// other unwanted keys).
	expected := ldstoreimpl.NewUnboundedSegmentMembershipFromKeys(expectedIncludes, expectedExcludes)
