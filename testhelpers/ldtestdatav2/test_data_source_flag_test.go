...
func verifyFlag(t *testing.T, configureFlag func(*FlagBuilder), expectedFlag *ldbuilders.FlagBuilder) {
	...
	dataSelector := mocks.NewMockDataSelector(subsystems.NoSelector())
	testDataSourceTest(t, func(p testDataSourceTestParams) {
		t.Helper()
		p.withDataSynchronizer(t, dataSelector, func(synchronizer subsystems.DataSynchronizer, resultChan <-chan subsystems.DataSynchronizerResult) {
			t.Helper()
			f := p.td.Flag("flagkey")
			configureFlag(f)
			p.td.Update(f)

			// Wait for the synchronizer to process the update
		...
		L:
			for {
				...
				case result, ok := <-resultChan:
					if !ok {
						t.Fatal("synchronizer result channel closed unexpectedly")
					}

					if result.ChangeSet == nil {
						t.Fatal("synchronizer returned nil ChangeSet")
					}

					if result.ChangeSet.IntentCode() == subsystems.IntentTransferChanges {
						changes := result.ChangeSet.Changes()
						m.In(t).Assert(changes[0].Object, m.JSONStrEqual(string(expectedJSON)))
						break L
					}

					fmt.Printf("synchronizer returned result: %v\n", result)
				case <-timer.C:
					t.Fatal("synchronizer did not return result within timeout")
				}
			}
		})
	})
...
func TestFlagConfigV2(t *testing.T) {
...
func TestFlagTargetsV2(t *testing.T) {
	basicString := func() *ldbuilders.FlagBuilder {
		return ldbuilders.NewFlagBuilder("flagkey").Version(1).On(true).Variations(threeStringValues...)
	}
...
func TestRuleConfigV2(t *testing.T) {
