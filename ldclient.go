...
func MakeCustomClient(sdkKey string, config Config, waitFor time.Duration) (*LDClient, error) {
	...
	if waitFor > 0 && !client.dataSystem.Offline() {
		...
		for {
			select {
			case <-closeWhenReady:
				if client.dataSystem.DataStatus() != datasystem.Refreshed {
...
func (client *LDClient) Initialized() bool {
	return client.dataSystem.DataStatus() == datasystem.Refreshed
}
...
func (client *LDClient) AllFlagsState(context ldcontext.Context, options ...flagstate.Option) flagstate.AllFlags {
	...
	if client.IsOffline() {
		...
		if client.dataSystem.DataStatus() == datasystem.Cached {
