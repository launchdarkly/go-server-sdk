// Package ldfiledatav2 allows the LaunchDarkly client to read feature flag data from a file.
//
// This is different from [github.com/launchdarkly/go-server-sdk/v7/ldtestdatav2.DataSource], which
...
//
//	fds := ldfiledatav2.DataSource().FilePaths("./test-data/my-flags.json")
//	config := ld.Config{
//		DataSystem: ldcomponents.DataSystem().Custom().Synchronizers(fds, nil)
//	}
...
// fails.
//
// When a reloader is configured, the data source reloads after change notifications stop
// for a short interval, so one save that produces many notifications loads one time. A
// file that is rewritten continuously, faster than that interval, does not reload until
// the writes pause.
//
