...
func TestHTTPConfigurationBuilder(t *testing.T) {
	basicConfig := subsystems.BasicClientContext{SDKKey: "test-key", InstanceID: "test-instance-id"}
	...
	t.Run("defaults", func(t *testing.T) {
		...
		// X-LaunchDarkly-Instance-Id is sourced from ClientContext, not generated by the builder.
		assert.Equal(t, "test-instance-id", headers.Get("X-LaunchDarkly-Instance-Id"))
	...
	t.Run("instance id header", func(t *testing.T) {
		t.Run("uses value from client context", func(t *testing.T) {
			bc := basicConfig
			bc.InstanceID = "supplied-from-context"
			c, err := HTTPConfiguration().Build(bc)
			require.NoError(t, err)
			...
			assert.Equal(t, "supplied-from-context", values[0])
		})

		t.Run("omitted when client context has no instance id", func(t *testing.T) {
			// Generation is the LDClient's responsibility, not the HTTP configuration
			// builder's. Direct callers that bypass LDClient (e.g. tests constructing
			// BasicClientContext by hand) get no header rather than a synthetic UUID.
			bc := basicConfig
			bc.InstanceID = ""
			c, err := HTTPConfiguration().Build(bc)
			require.NoError(t, err)

			assert.Empty(t, c.DefaultHeaders.Values("X-LaunchDarkly-Instance-Id"))
		})
	})
